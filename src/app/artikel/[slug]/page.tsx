...
import type { Article as MockArticle, Author as MockAuthor } from "@/types/author";
...
const BANKING: ArticleData = {
  ...
  body: "banking",
  related: [
    { category: "KI im Business", title: "Wie Schweizer KMU bei Edge-AI vorne dabei sind", author: "Ali Soy", date: "28.04.2026", image: "https://picsum.photos/seed/article1/600/400", href: "/artikel/schweizer-kmu-edge-ai" },
    { category: "Future of Work", title: "Die Zukunft der Arbeit: Augmentation statt Automation", author: "Ali Soy", date: "15.04.2026", image: "https://picsum.photos/seed/article5/600/400", href: "/artikel/zukunft-arbeit-augmentation" },
...
export default async function ArticlePage({ params }: PageProps) {
  ...
  const hardcoded = ARTICLES[slug];
  if (hardcoded) {
    return <HardcodedArticleView article={hardcoded} />;
  }
  const mock = getPublishedArticleBySlug(slug);
  if (!mock) notFound();
  const author = getAuthor(mock.authorId);
  if (!author) notFound();
  return <MockArticleView article={mock} author={author} />;
}

function HardcodedArticleView({ article }: { article: ArticleData }) {
...
function MockArticleView({ article, author }: { article: MockArticle; author: MockAuthor }) {
  ...
  return (
    <main style={{ paddingTop: "var(--nav-h)", backgroundColor: "var(--da-dark)", minHeight: "100vh" }}>
      ...
      <div style={{ maxWidth: "860px", margin: "0 auto", padding: "0 var(--sp-8) var(--sp-12)" }}>
        <article>
          <ArticleBody>
