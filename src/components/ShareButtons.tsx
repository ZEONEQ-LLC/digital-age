...
export default function ShareButtons({ title, url }: { title: string; url: string }) {
  ...
  // `url` MUSS absolut sein (vom Server-Caller via getBaseUrl prefixiert).
  // Kein window.location-Fallback mehr — sonst landet beim Copy auf Preview-
  // Deploys die Hash-Vercel-URL im Zwischenspeicher.
  const shareUrl = url;
  const encodedUrl = encodeURIComponent(shareUrl);
  const encodedTitle = encodeURIComponent(title);
  // Email-Body: Title + Leerzeile + Link. encodeURIComponent macht den
  // Newline (\n) korrekt zu %0A.
  ...
  return (
    <>
      ...
      <div className="share-row">
        <span className="share-row__label">Teilen</span>
        <a
        ...
        <button
          type="button"
          onClick={handleCopy}
          className={`share-btn${copied ? " share-btn--active" : ""}`}
