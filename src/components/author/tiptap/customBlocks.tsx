// Tiptap-Custom-Extensions für den Author-Editor (Etappe A):
//   - StatBoxBlock     — strukturierte Statistik-Box mit React-NodeView
//   - DaDividerBlock   — Trennlinie mit variant-Attribut
//   - SourceRefInline  — `[^N]` als read-only Inline-Node (Etappe A:
//                        nur sichtbar machen, Picker in Etappe B)
//
// Node-Namen matchen exakt den Phase-1-Konverter (src/lib/tiptap/types.ts):
// daStatBox, daDivider, daSourceRef. Damit klappt der Roundtrip Block[] →
// Tiptap-Doc → Block[] ohne Mapping-Lücken.

import { Node, mergeAttributes } from "@tiptap/core";
import { ReactNodeViewRenderer, NodeViewWrapper } from "@tiptap/react";
import type { StatBoxItem } from "@/types/blocks";
import StatBoxNodeView from "./StatBoxNodeView";
import SourceRefNodeView from "./SourceRefNodeView";
...
declare module "@tiptap/core" {
  interface Commands<ReturnType> {
    ...
    daSourceRef: {
      insertSourceRef: (n: number) => ReturnType;
    };
  }
}
...
export const SourceRefInline = Node.create({
  name: "daSourceRef",
  group: "inline",
  inline: true,
  atom: true,
  ...
  addAttributes() {
    return {
      n: {
        default: 0,
        parseHTML: (el: HTMLElement) => parseInt(el.getAttribute("data-n") ?? "0", 10),
        renderHTML: (attrs: { n?: number }) => ({ "data-n": String(attrs.n ?? 0) }),
      },
    };
  },
  parseHTML() {
    return [{ tag: 'sup[data-type="source-ref"]' }];
  },
  renderHTML({ HTMLAttributes }) {
    return ["sup", mergeAttributes(HTMLAttributes, { "data-type": "source-ref" })];
  },
  addNodeView() {
    return ReactNodeViewRenderer(SourceRefNodeView);
  },
  addCommands() {
    return {
      insertSourceRef:
